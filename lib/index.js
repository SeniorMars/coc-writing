...
function buildDeleteIndex(maxDistance) {
  ...
  for (const lemma of sortedKeys) {
    ...
    for (const del of editsDeletes(lemma, maxDistance)) {
      const existing = tmp.get(del) ?? /* @__PURE__ */ new Set();
      existing.add(lemma);
...
function getSpellingSuggestions(input, limit, maxDistance = 2) {
  ...
  for (const lemma of candidateSet) {
    ...
    const score = distance * 100 - prefixBonus * 5 + lengthPenalty * 2 + punctuationPenalty;
    ...
    doComplete: async function(opt) {
      ...
      const items = lemmas.filter((lemma) => {
        ...
        return {
          ...
          kind: import_coc.CompletionItemKind.Text,
          info: "",
          data: { lemma }
        };
      });
    ...
    doComplete: async function(opt) {
      ...
      const items = Array.from(synonymMap.entries()).map(([lemmaKey], i) => {
        ...
        return {
          ...
          sortText: String(i).padStart(5, "0"),
          kind: import_coc2.CompletionItemKind.Text,
          info: "",
...
var ThesaurusList = class extends import_coc3.BasicList {
  ...
  async loadItems(context) {
    ...
    const cfg = import_coc3.workspace.getConfiguration("coc-writing");
    const simPointers = cfg.get("thesaurus.similarityPointers", [
      "&",
      "^",
      "+"
    ]);
...
async function activate(context) {
  ...
  context.subscriptions.push(
    import_coc4.sources.createSource(createDictionarySource(filetypes)),
    import_coc4.sources.createSource(createThesaurusSource(filetypes)),
    ...
    import_coc4.commands.registerCommand("coc-writing.health", async () => {
      ...
      const lines = [
        ...
        `similarityPointers: ${cfg.get("thesaurus.similarityPointers", ["&", "^", "+"]).join(" ")}`
      ];
